...
def options(opt):
    ...
    optgrp = opt.add_option_group('lvs-cxx Options')
...
def configure(conf):
    ...
    conf.write_config_header('src/lvs-cxx-config.hpp', define_prefix='LVS_CXX_')

def build(bld):
    bld.shlib(target='lvs-cxx',
              vnum=VERSION,
              cnum=VERSION,
    ...
    bld(features='subst',
        source='liblvs-cxx.pc.in',
        target='liblvs-cxx.pc',
    ...
    bld.install_files('${INCLUDEDIR}/lvs-cxx',
                      bld.path.find_resource('src/lvs-cxx-config.hpp'))